...
func ListAudits(c *gin.Context) {
	...
	defer db.Close()
	var audits []utils.Tasks_Audit
	// Extract the user ID associated with the current session
	if claims, ok := token.Claims.(jwt.MapClaims); ok && token.Valid {
		userId := claims["sub"]

		rows, err := db.Query("SELECT id, title, description, created_at, updated_at, user_id, issue_type, assignee, sprint_id, project_id, points,reporter, comments, task_id, status FROM tasks_audit WHERE user_id=?", userId)
		if err != nil {
		...
		for rows.Next() {
			task := utils.Tasks_Audit{}
			var createdAt, updatedAt string
			err := rows.Scan(&task.ID, &task.Title, &task.Description, &createdAt, &updatedAt, &task.UserID, &task.IssueType, &task.Assignee, &task.Sprint, &task.ProjectID, &task.Points, &task.Reporter, &task.Comments, &task.TaskID, &task.Status)
			if err != nil {
